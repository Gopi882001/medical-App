...
export const consoleLog = (
  ...
  // if(APP_ENV() != Environments.prod)
  console.log('[' + screen + ']->[' + method + ']:  ' + title + ': ', message);
...
export const getAutoEventId = async () => {
  ...
  consoleLog('PMUtils', 'getAutoEventId', 'eventId:', 'value:' + value);
  return value;
};

export const getAutoEventMessage = async () => {
  const value = await NativeModules.NativeBridge.getAutoEventMessage();
  consoleLog('PMUtils', 'getAutoEventId', 'eventMessage:', 'value:' + value);
  return value;
};
...
export const getTotalRAMSize = async () => {
  ...
  consoleLog('PMUtils', 'getTotalRAMSize', 'totalRAMSize', totalRAMSize);
  return totalRAMSize;
};
...
export const takeNotificationPermission = (callback) => {
  NativeModules.NativeBridge.takeNotificationPermission(
    (granted, error = null) => {
      consoleLog('PMUtils', 'takeNotificationPermission', 'granted', granted);
      if (error != null) {
        consoleLog('PMUtils', 'takeNotificationPermission', 'error', error);
      }
      if (callback && typeof callback === 'function') {
...
export const getTwitchChanelFromUrl = (url) => {
  consoleLog('PMUtils', 'getTwitchChanelFromUrl', 'url', url);
