...
+ (NSString*) _getOpenUDID {
    ...
    if (_openUDID==nil) {
        ...
        _openUDID = [NSString stringWithFormat:
                ...
                (uint)(arc4random() % 4294967295)];
        
        
        unsigned char digest[CC_SHA1_DIGEST_LENGTH];
        NSData *stringBytes = [_openUDID dataUsingEncoding: NSUTF8StringEncoding]; /* or some other encoding */
...
//
+ (NSString*) value {
    return [NUOpenUDID valueWithError:nil];
}
+ (NSString*) valueWithError:(NSError **)error {
    ...
    if(guuid == nil)
    {
      ...
      guuid = (NSString *) CFBridgingRelease(CFUUIDCreateString(NULL, uuid));
      CFRelease(uuid);
    }
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [NUOpenUDID _getDictFromPasteboard:slotPB];
      //NSLog(@"Other app open udid dictionary:%@",dict);
    ...
    //
  //NSLog(@"Total found openudids:%@",frequencyDict);
    ...
    if (openUDID==nil) {
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [NUOpenUDID _getOpenUDID];
        } else {
...
//        [slotPB setPersistent:YES];
        ...
        if (openUDID && localDict)
            [NUOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
